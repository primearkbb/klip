...
export class AutocompleteInput {
  ...
  private showSuggestions(): void {
    ...
    if (this.showingSuggestions) {
      ...
      Deno.stdout.write(
        new TextEncoder().encode(`\x1b[${this.suggestions.length + 1}A`),
      );
    }
      ...
      if (isSelected) {
        Deno.stdout.write(
          new TextEncoder().encode(
            colors.bgBlue(colors.white(`  ${suggestion}  `)) + '\n',
          ),
        );
      } else {
        Deno.stdout.write(
          new TextEncoder().encode(colors.dim(`  ${suggestion}`) + '\n'),
        );
      }
    }
    ...
    Deno.stdout.write(
      new TextEncoder().encode(`\x1b[${this.suggestions.length}A`),
    );
  ...
  private clearSuggestions(): void {
    if (this.showingSuggestions) {
      ...
      Deno.stdout.write(
        new TextEncoder().encode(`\x1b[${this.suggestions.length}B`),
      );
